...
-- PST_OP_TYPE_TB Table Create SQL
CREATE TABLE PST_OP_TYPE_TB
(
    `PST_OP_TYPE_PK`     VARCHAR(45)     NOT NULL    COMMENT 'NORMAL, NOTICE, PRIVATE, SECRET, HIDDEN, LOCK, BLIND', 
    `PST_OP_TYPE_EXPLN`  VARCHAR(300)    NOT NULL    COMMENT '게시글 공개 유형 설명', 
    PRIMARY KEY (PST_OP_TYPE_PK)
);

ALTER TABLE PST_OP_TYPE_TB COMMENT '게시글 공개 유형 테이블';
...
(
    ...
    `PST_OP_TYPE_FK`   VARCHAR(45)    NOT NULL    COMMENT '파일 공개 유형 (NORMAL, NOTICE, PRIVATE, SECRET, HIDDEN, LOCK, BLIND)', 
...
ALTER TABLE FILE_GRP_TB
    ADD CONSTRAINT FK_FILE_GRP_TB_MEM_FK_MEM_TB_MEM_SQ_PK FOREIGN KEY (MEM_FK)
        REFERENCES MEM_TB (MEM_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE FILE_GRP_TB
    ADD CONSTRAINT FK_FILE_GRP_TB_PST_OP_TYPE_FK_PST_OP_TYPE_TB_PST_OP_TYPE_PK FOREIGN KEY (PST_OP_TYPE_FK)
        REFERENCES PST_OP_TYPE_TB (PST_OP_TYPE_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
(
    ...
    `MEM_FK`              INT        NOT NULL    COMMENT '댓글 그룹 생성자 (비밀댓글을 볼 수 있는 자)', 
...
ALTER TABLE CMT_GRP_TB
    ADD CONSTRAINT FK_CMT_GRP_TB_MEM_FK_MEM_TB_MEM_SQ_PK FOREIGN KEY (MEM_FK)
        REFERENCES MEM_TB (MEM_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;


-- BRD_TB Table Create SQL
CREATE TABLE BRD_TB
(
    `BRD_SQ_PK`   INT            NOT NULL    AUTO_INCREMENT COMMENT '게시판 시퀀스 기본키', 
    `BRD_FL`      TINYINT        NOT NULL    DEFAULT 0 COMMENT '게시판 사용여부', 
    `BRD_REG_DT`  TIMESTAMP      NOT NULL    COMMENT '게시판 등록 날짜', 
    `RES_SQ_PK`   INT            NULL        COMMENT '게시판 URL (참조용)', 
    `BRD_NM_UNQ`  VARCHAR(50)    NOT NULL    COMMENT '게시판 이름 유니크', 
    `BRD_EXPLN`   VARCHAR(50)    NOT NULL    COMMENT '게시판 설명', 
    PRIMARY KEY (BRD_SQ_PK)
);

ALTER TABLE BRD_TB COMMENT '게시판 테이블';

ALTER TABLE BRD_TB
    ADD CONSTRAINT FK_BRD_TB_RES_SQ_PK_RES_TB_RES_SQ_PK FOREIGN KEY (RES_SQ_PK)
        REFERENCES RES_TB (RES_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;


-- PST_GRP_TB Table Create SQL
CREATE TABLE PST_GRP_TB
(
    `PST_GRP_SQ_PK`   INT            NOT NULL    AUTO_INCREMENT COMMENT '게시글 묶음 시퀀스 기본키', 
    `BRD_FK`          INT            NOT NULL    COMMENT '게시판 외래키', 
    `MEM_FK`          INT            NOT NULL    COMMENT '계정 외래키', 
    `CMT_GRP_FK`      INT            NOT NULL    COMMENT '댓글 묶음 외래키', 
    `PST_OP_TYPE_FK`  VARCHAR(45)    NOT NULL    COMMENT 'NORMAL, NOTICE, PRIVATE, SECRET, HIDDEN, LOCK, BLIND', 
    `PST_GRP_REG_DT`  TIMESTAMP      NOT NULL    DEFAULT current_timestamp() COMMENT '게시글 묶음 등록 날짜', 
    `HITS`            INT            NOT NULL    DEFAULT 0 COMMENT '조회수', 
    PRIMARY KEY (PST_GRP_SQ_PK)
);

ALTER TABLE PST_GRP_TB COMMENT '게시글 묶음 테이블';

ALTER TABLE PST_GRP_TB
    ADD CONSTRAINT FK_PST_GRP_TB_MEM_FK_MEM_TB_MEM_SQ_PK FOREIGN KEY (MEM_FK)
        REFERENCES MEM_TB (MEM_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE PST_GRP_TB
    ADD CONSTRAINT FK_PST_GRP_TB_CMT_GRP_FK_CMT_GRP_TB_CMT_GRP_SQ_PK FOREIGN KEY (CMT_GRP_FK)
        REFERENCES CMT_GRP_TB (CMT_GRP_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE PST_GRP_TB
    ADD CONSTRAINT FK_PST_GRP_TB_PST_OP_TYPE_FK_PST_OP_TYPE_TB_PST_OP_TYPE_PK FOREIGN KEY (PST_OP_TYPE_FK)
        REFERENCES PST_OP_TYPE_TB (PST_OP_TYPE_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE PST_GRP_TB
    ADD CONSTRAINT FK_PST_GRP_TB_BRD_FK_BRD_TB_BRD_SQ_PK FOREIGN KEY (BRD_FK)
        REFERENCES BRD_TB (BRD_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;


-- CMT_OP_TYPE_TB Table Create SQL
CREATE TABLE CMT_OP_TYPE_TB
(
    `CMT_OP_TYPE_PK`     VARCHAR(45)     NOT NULL    COMMENT 'NORMAL, SECRET, HIDDEN, LOCK, BLIND', 
    `CMT_OP_TYPE_EXPLN`  VARCHAR(300)    NOT NULL    COMMENT '댓글 공개 유형 설명', 
    PRIMARY KEY (CMT_OP_TYPE_PK)
);

ALTER TABLE CMT_OP_TYPE_TB COMMENT '댓글 공개 유형 테이블';
...
ALTER TABLE MEM_AUTH_TB
    ADD CONSTRAINT FK_MEM_AUTH_TB_MEM_FK_PK_MEM_TB_MEM_SQ_PK FOREIGN KEY (MEM_FK_PK)
        REFERENCES MEM_TB (MEM_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE MEM_AUTH_TB
    ADD CONSTRAINT FK_MEM_AUTH_TB_AUTH_FK_PK_AUTH_TB_AUTH_SQ_PK FOREIGN KEY (AUTH_FK_PK)
...
(
    ...
    `AUTH_FK_PK`  INT    NOT NULL    COMMENT '권한 외래키 기본키', 
    `PRIORITY`    INT    NOT NULL    DEFAULT 0 COMMENT '숫자가 높을수록 더 강력한 권한', 
    PRIMARY KEY (RES_FK_PK, AUTH_FK_PK)
);

ALTER TABLE RES_AUTH_TB COMMENT '리소스-권한 테이블(복합키)';

ALTER TABLE RES_AUTH_TB
    ADD CONSTRAINT FK_RES_AUTH_TB_RES_FK_PK_RES_TB_RES_SQ_PK FOREIGN KEY (RES_FK_PK)
        REFERENCES RES_TB (RES_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE RES_AUTH_TB
    ADD CONSTRAINT FK_RES_AUTH_TB_AUTH_FK_PK_AUTH_TB_AUTH_SQ_PK FOREIGN KEY (AUTH_FK_PK)
        REFERENCES AUTH_TB (AUTH_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
ALTER TABLE BLOCK_TB
    ADD CONSTRAINT FK_BLOCK_TB_BLOCK_TRGT_MEM_FK_MEM_TB_MEM_SQ_PK FOREIGN KEY (BLOCK_TRGT_MEM_FK)
        REFERENCES MEM_TB (MEM_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE BLOCK_TB
    ADD CONSTRAINT FK_BLOCK_TB_BLOCK_REG_MEM_FK_MEM_TB_MEM_SQ_PK FOREIGN KEY (BLOCK_REG_MEM_FK)
        REFERENCES MEM_TB (MEM_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
ALTER TABLE FILE_TB
    ADD CONSTRAINT FK_FILE_TB_FILE_GRP_FK_FILE_GRP_TB_FILE_GRP_SQ_PK FOREIGN KEY (FILE_GRP_FK)
        REFERENCES FILE_GRP_TB (FILE_GRP_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE FILE_TB
    ADD CONSTRAINT FK_FILE_TB_MEM_FK_MEM_TB_MEM_SQ_PK FOREIGN KEY (MEM_FK)
        REFERENCES MEM_TB (MEM_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;


-- PST_TB Table Create SQL
CREATE TABLE PST_TB
(
    `PST_SQ_PK`    INT           NOT NULL    AUTO_INCREMENT COMMENT '게시글 시퀀스 기본키', 
    `PST_GRP_FK`   INT           NOT NULL    COMMENT '게시글 묶음 외래키', 
    `FILE_GRP_FK`  INT           NOT NULL    COMMENT '파일 묶음 외래키', 
    `PST_REG_DT`   TIMESTAMP     NOT NULL    DEFAULT current_timestamp() COMMENT '게시글 등록 날짜', 
    `PST_TITLE`    TINYTEXT      NOT NULL    COMMENT '게시글 제목', 
    `PST_BODY`     MEDIUMTEXT    NOT NULL    COMMENT '게시글 본문', 
    PRIMARY KEY (PST_SQ_PK)
);

ALTER TABLE PST_TB COMMENT '게시글 상세 테이블';

ALTER TABLE PST_TB
    ADD CONSTRAINT FK_PST_TB_PST_GRP_FK_PST_GRP_TB_PST_GRP_SQ_PK FOREIGN KEY (PST_GRP_FK)
        REFERENCES PST_GRP_TB (PST_GRP_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE PST_TB
    ADD CONSTRAINT FK_PST_TB_FILE_GRP_FK_FILE_GRP_TB_FILE_GRP_SQ_PK FOREIGN KEY (FILE_GRP_FK)
        REFERENCES FILE_GRP_TB (FILE_GRP_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
(
    ...
    `CMT_GRP_FK`      INT            NOT NULL    COMMENT '댓글 묶음 외래키', 
    `MEM_FK`          INT            NOT NULL    COMMENT '댓글 작성자', 
    `FILE_GRP_FK`     INT            NULL        COMMENT '파일 묶음 외래키', 
    `CMT_OP_TYPE_FK`  VARCHAR(45)    NOT NULL    COMMENT 'NORMAL, SECRET, HIDDEN, LOCK, BLIND', 
...
ALTER TABLE CMT_TB
    ADD CONSTRAINT FK_CMT_TB_CMT_GRP_FK_CMT_GRP_TB_CMT_GRP_SQ_PK FOREIGN KEY (CMT_GRP_FK)
        REFERENCES CMT_GRP_TB (CMT_GRP_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE CMT_TB
    ADD CONSTRAINT FK_CMT_TB_MEM_FK_MEM_TB_MEM_SQ_PK FOREIGN KEY (MEM_FK)
        REFERENCES MEM_TB (MEM_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE CMT_TB
    ADD CONSTRAINT FK_CMT_TB_FILE_GRP_FK_FILE_GRP_TB_FILE_GRP_SQ_PK FOREIGN KEY (FILE_GRP_FK)
        REFERENCES FILE_GRP_TB (FILE_GRP_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE CMT_TB
    ADD CONSTRAINT FK_CMT_TB_CMT_OP_TYPE_FK_CMT_OP_TYPE_TB_CMT_OP_TYPE_PK FOREIGN KEY (CMT_OP_TYPE_FK)
        REFERENCES CMT_OP_TYPE_TB (CMT_OP_TYPE_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
ALTER TABLE TEMP_SAVE_TB
    ADD CONSTRAINT FK_TEMP_SAVE_TB_MEM_FK_MEM_TB_MEM_SQ_PK FOREIGN KEY (MEM_FK)
        REFERENCES MEM_TB (MEM_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE TEMP_SAVE_TB
    ADD CONSTRAINT FK_TEMP_SAVE_TB_FILE_GRP_FK_FILE_GRP_TB_FILE_GRP_SQ_PK FOREIGN KEY (FILE_GRP_FK)
        REFERENCES FILE_GRP_TB (FILE_GRP_SQ_PK) ON DELETE RESTRICT ON UPDATE RESTRICT;
