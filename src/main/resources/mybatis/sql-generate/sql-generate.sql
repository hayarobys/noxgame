...
-- OPEN_TYPE_TB Table Create SQL
CREATE TABLE OPEN_TYPE_TB
(
    `OPEN_TYPE_PK`     VARCHAR(45)     NOT NULL    COMMENT 'PUBLIC, MEMBER, SECRET, CLOSE', 
    `OPEN_TYPE_EXPLN`  VARCHAR(300)    NOT NULL    COMMENT '공개 유형 설명', 
    PRIMARY KEY (OPEN_TYPE_PK)
);

ALTER TABLE OPEN_TYPE_TB COMMENT '공개 유형 테이블';
...
(
    ...
    `OPEN_TYPE_FK`     VARCHAR(45)    NOT NULL    COMMENT 'PUBLIC, MEMBER, SECRET, CLOSE', 
...
ALTER TABLE FILE_GRP_TB ADD CONSTRAINT FK_FILE_GRP_TB_OPEN_TYPE_FK_OPEN_TYPE_TB_OPEN_TYPE_PK FOREIGN KEY (OPEN_TYPE_FK)
 REFERENCES OPEN_TYPE_TB (OPEN_TYPE_PK)  ON DELETE RESTRICT ON UPDATE RESTRICT;
...
(
    `AUTH_GRP_PK`     VARCHAR(45)     NOT NULL    COMMENT 'SYSTEM_OPERATOR, MEMBER', 
    `AUTH_GRP_EXPLN`  VARCHAR(300)    NOT NULL    COMMENT '권한 묶음 설명', 
    PRIMARY KEY (AUTH_GRP_PK)
);

ALTER TABLE AUTH_GRP_TB COMMENT '권한 그룹 테이블';
...
(
    ...
    `OPEN_TYPE_FK`      VARCHAR(45)    NOT NULL    COMMENT 'PUBLIC, MEMBER, SECRET, CLOSE', 
    `FRBRD_GRP_REG_DT`  TIMESTAMP      NOT NULL    DEFAULT current_timestamp() COMMENT '자유게시판 묶음 등록 날짜', 
...
ALTER TABLE FRBRD_GRP_TB ADD CONSTRAINT FK_FRBRD_GRP_TB_OPEN_TYPE_FK_OPEN_TYPE_TB_OPEN_TYPE_PK FOREIGN KEY (OPEN_TYPE_FK)
 REFERENCES OPEN_TYPE_TB (OPEN_TYPE_PK)  ON DELETE RESTRICT ON UPDATE RESTRICT;
...
(
    ...
    `FRBRD_GRP_FK`  INT           NOT NULL    COMMENT '자유게시판 묶음 외래키', 
    `FILE_GRP_FK`   INT           NOT NULL    COMMENT '파일 묶음 외래키', 
...
ALTER TABLE CMT_TB ADD CONSTRAINT FK_CMT_TB_FILE_GRP_FK_FILE_GRP_TB_FILE_GRP_SQ_PK FOREIGN KEY (FILE_GRP_FK)
 REFERENCES FILE_GRP_TB (FILE_GRP_SQ_PK)  ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE CMT_TB ADD CONSTRAINT FK_CMT_TB_OPEN_TYPE_FK_OPEN_TYPE_TB_OPEN_TYPE_PK FOREIGN KEY (OPEN_TYPE_FK)
 REFERENCES OPEN_TYPE_TB (OPEN_TYPE_PK)  ON DELETE RESTRICT ON UPDATE RESTRICT;
...
-- MEMDTA_TB Table Create SQL
CREATE TABLE MEMDTA_TB
(
    `MEMDTA_SQ_PK`     INT            NOT NULL    AUTO_INCREMENT COMMENT '회원자료실 시퀀스 기본키', 
    `MEM_FK`           INT            NOT NULL    COMMENT '계정 외래키', 
    `FILE_GRP_FK`      INT            NOT NULL    COMMENT '파일 묶음 외래키', 
    `CMT_GRP_FK`       INT            NOT NULL    COMMENT '댓글 묶음 외래키', 
    `OPEN_TYPE_FK`     VARCHAR(45)    NOT NULL    COMMENT 'PUBLIC, MEMBER, SECRET, CLOSE', 
    `MEMDTA_CLS_FK`    INT            NULL        COMMENT '계층의 부모 키 default는 자기 자신의 게시글 그룹 일련 번호와 동일', 
    `MEMDTA_CLS_ORD`   INT            NOT NULL    DEFAULT 0 COMMENT '계층 내 정렬순서 default 0', 
    `MEMDTA_CLS_DPTH`  INT            NOT NULL    DEFAULT 1 COMMENT '계층 내 깊이 default 1', 
    `MEMDTA_REG_DT`    TIMESTAMP      NOT NULL    DEFAULT current_timestamp() COMMENT '회원자료실 등록 날짜', 
    `MEMDTA_MOD_DT`    TIMESTAMP      NOT NULL    DEFAULT current_timestamp() COMMENT '회원자료실 수정 날짜', 
    `MEMDTA_TITLE`     TINYTEXT       NOT NULL    COMMENT '회원자료실 제목', 
    `MEMDTA_BODY`      MEDIUMTEXT     NOT NULL    COMMENT '회원자료실 본문', 
    `HITS`             INT            NOT NULL    DEFAULT 0 COMMENT '조회수', 
    PRIMARY KEY (MEMDTA_SQ_PK)
);

ALTER TABLE MEMDTA_TB COMMENT '회원자료실 테이블';

ALTER TABLE MEMDTA_TB ADD CONSTRAINT FK_MEMDTA_TB_MEM_FK_MEM_TB_MEM_SQ_PK FOREIGN KEY (MEM_FK)
 REFERENCES MEM_TB (MEM_SQ_PK)  ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE MEMDTA_TB ADD CONSTRAINT FK_MEMDTA_TB_CMT_GRP_FK_CMT_GRP_TB_CMT_GRP_SQ_PK FOREIGN KEY (CMT_GRP_FK)
 REFERENCES CMT_GRP_TB (CMT_GRP_SQ_PK)  ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE MEMDTA_TB ADD CONSTRAINT FK_MEMDTA_TB_FILE_GRP_FK_FILE_GRP_TB_FILE_GRP_SQ_PK FOREIGN KEY (FILE_GRP_FK)
 REFERENCES FILE_GRP_TB (FILE_GRP_SQ_PK)  ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE MEMDTA_TB ADD CONSTRAINT FK_MEMDTA_TB_OPEN_TYPE_FK_OPEN_TYPE_TB_OPEN_TYPE_PK FOREIGN KEY (OPEN_TYPE_FK)
 REFERENCES OPEN_TYPE_TB (OPEN_TYPE_PK)  ON DELETE RESTRICT ON UPDATE RESTRICT;
